...
int main() {
	...
	while(1) {
		...
		// set up listening to sockets
		if(select(max_fd+1,&read_fdset,NULL,NULL,NULL)<0) {
			perror("select");
			exit (-1);
		}

		// Loop through sockets to listen for new activity
		for(int fd = 3 ; fd<=max_fd; fd++) {
		
			// If socket is active
			if(FD_ISSET(fd,&read_fdset)) {

				// If active socket is server's, accept connection
				if(fd==server_sd) {
...
void FD_COPY(void* dest, void* src) {
